...
import { formatNumber, toLookup } from "@/lib/utils";
  ...
  return (
    <div key={`conversion-${conv.key}-outer`} className="min-h-10 pt-3">
      ...
      <span className="text-stock-unit">
        {toValue ? formatNumber(Number(toValue),grocyConfig) : "???"} {Number(toValue) !== 1.0 ? unitFrom.name_plural : unitFrom.name}
      </span>{" "}
      to{" "}
      ...
      <span className="text-slate-400">«=»</span> {formatNumber(convFactor,grocyConfig)}{" "}
